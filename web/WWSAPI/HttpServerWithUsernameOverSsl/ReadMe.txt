...
This example shows a TCP server that accepts a channel and receives
and replies to messages, with username over SSL mixed-mode security.
In this setup, the transport connection is protected (signed,
encrypted) by SSL which also provides server authentication.  Client
authentication is provided by a username/password pair in a
...
To build the HttpServerWithUsernameOverSslExample sample
  1. Open the solution HttpServerWithUsernameOverSslExample.sln in Visual Studio. 
  2. On the Build menu, click Build. 

Running the Sample
==================

To run the HttpServerWithUsernameOverSslExample sample
  1. Run HttpServerWithUsernameOverSslExample by clicking Start Without Debugging on the Debug menu.
