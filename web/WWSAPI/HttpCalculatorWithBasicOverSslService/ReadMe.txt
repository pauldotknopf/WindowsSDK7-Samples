...
This example shows how to use service host for hosting a calculator
service over HTTP, with SSL transport security and HTTP header basic
authentication.  In this setup, the transport connection is protected
(signed, encrypted) by SSL which also provides server authentication.
Client authentication is provided by a username/password pair supplied
in HTTP headers using the basic scheme.
...
To build the HttpCalculatorWithBasicOverSslServiceExample sample
  1. Open the solution HttpCalculatorWithBasicOverSslServiceExample.sln in Visual Studio. 
  2. On the Build menu, click Build. 

Running the Sample
==================

To run the HttpCalculatorWithBasicOverSslServiceExample sample
  1. Run HttpCalculatorWithBasicOverSslServiceExample by clicking Start Without Debugging on the Debug menu.
