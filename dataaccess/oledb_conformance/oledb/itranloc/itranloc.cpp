...
int TCITransactionObject::Variation_1()
{
	...
	ITransaction		**ppITransaction[ulNestedTnxs];
	ULONG				i								= 0;
	ULONG				ulTransactionLevel				= 0;

	for (i=0;i<ulNestedTnxs;i++)
	{
		ppITransaction[i]	= NULL;
	}
	...
	for (i=0;i<ulNestedTnxs;i++)
	{
		...
		if (!CHECK(pITransactionObject->GetTransactionObject(ulTransactionLevel,ppITransaction[i]),S_OK))
		{
			goto CLEANUP;
		}
	}
	...
	if (CHECK((*ppITransaction[8])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK)){goto CLEANUP;}
	//since 8 was committed 9 is implicity committed as well
	if (CHECK((*ppITransaction[9])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), XACT_E_NOTRANSACTION)){goto CLEANUP;}

	if (CHECK((*ppITransaction[7])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK)){goto CLEANUP;}
	if (CHECK((*ppITransaction[6])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK)){goto CLEANUP;}
	if (CHECK((*ppITransaction[5])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK)){goto CLEANUP;}
	if (CHECK((*ppITransaction[4])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK)){goto CLEANUP;}
	if (CHECK((*ppITransaction[3])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK)){goto CLEANUP;}
	if (CHECK((*ppITransaction[0])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK)){goto CLEANUP;}
	//since 0 was committed 1 and 2 are implicity committed as well
	if (CHECK((*ppITransaction[2])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK)){goto CLEANUP;}
	if (CHECK((*ppITransaction[1])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK)){goto CLEANUP;}
...
CLEANUP:
	...
	for (i=0;i<ulNestedTnxs;i++)
	{
		if (ppITransaction[i])
		{
			PROVIDER_FREE(*ppITransaction[i]);
		}
	}
	PROVIDER_FREE(pITransactionObject);
	return fResults;
